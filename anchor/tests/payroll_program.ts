...
describe('Payroll Program - Comprehensive Tests', () => {
  ...
  describe('4. Payroll Processing (process_payroll)', () => {
    // FIX 1: Use a fixed past timestamp so it won't collide with the
    // "already paid" re-run test below, and is clearly a unique cycle.
    const cycleTimestamp = new BN(1_000_000)
    ...
    it('Should skip workers already paid in current cycle', async () => {
      ...
      // FIX 2: Re-use the exact same cycleTimestamp so all workers are
      // considered "already paid" and the treasury must stay unchanged.
      await program.methods
        .processPayroll(cycleTimestamp)
        .accountsPartial({
          org: orgPda,
  ...
  describe('6. Integration & Edge Cases', () => {
    it('Should handle complete payroll lifecycle', async () => {
      ...
      const lWorker2 = Keypair.generate()
      ...
      const [lWorker2Pda] = PublicKey.findProgramAddressSync(
        [
          Buffer.from('worker'),
          lifecycleOrgPda.toBuffer(),
          lWorker2.publicKey.toBuffer(),
        ],
        program.programId
      )
      ...
      // FIX 3: The org's workers_count is 2 after adding lWorker2, so we
      // must pass exactly 2 workers in remainingAccounts to avoid the
      // "index out of bounds" panic in the Rust program.
      await program.methods
        .addWorker(new BN(LAMPORTS_PER_SOL))
        .accountsPartial({
          org: lifecycleOrgPda,
          workerPubkey: lWorker2.publicKey,
          authority: authority.publicKey,
          systemProgram: anchor.web3.SystemProgram.programId,
        })
        .rpc()

      // Fund treasury with enough for both workers
      ...
      // Process payroll — pass all 2 workers
      await program.methods
        .processPayroll(new BN(2_000_000))
        .accountsPartial({
          org: lifecycleOrgPda,
          authority: authority.publicKey,
          systemProgram: anchor.web3.SystemProgram.programId,
        ...
        .remainingAccounts([
          ...
          { pubkey: lWorker2Pda, isSigner: false, isWritable: true },
          { pubkey: lWorker2.publicKey, isSigner: false, isWritable: true },
        ])
        .rpc()
